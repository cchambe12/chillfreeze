...
chill.stan <- subset(chill.stan, select=c("RGR.stand", "tx", "chill1", "chill2", "species"))
...
sumer.ni <- summary(htdiff.inter.ncp)$summary
sumer.ni[grep("mu_", rownames(sumer.ni)),]

sort(unique(chill.stan$species)) # numbers are alphabetical


modelhere <- htdiff.inter.ncp
quartz()
muplotfx(modelhere, "", 8, 8, c(0,5), c(-15, 18) , 19.5, 3.5)
