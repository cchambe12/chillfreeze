...
model {
	...
	a_sp ~ student_t(mu_a_sp, sigma_a_sp); 
	b_tx ~ student_t(mu_b_tx_sp, sigma_b_tx_sp); 
	b_chill1 ~ student_t(mu_b_chill1_sp, sigma_b_chill1_sp); 
	b_chill2 ~ student_t(mu_b_chill2_sp, sigma_b_chill2_sp);
	b_txchill1 ~ student_t(mu_b_txchill1_sp, sigma_b_txchill1_sp);
	b_txchill2 ~ student_t(mu_b_txchill2_sp, sigma_b_txchill2_sp); 

        mu_a_sp ~ student_t(0, 50);
        sigma_a_sp ~ student_t(0, 10);

        mu_b_tx_sp ~ student_t(0, 50);
        sigma_b_tx_sp ~ student_t(0, 10);
        mu_b_chill1_sp ~ student_t(0, 50);
        sigma_b_chill1_sp ~ student_t(0, 10);
        mu_b_chill2_sp ~ student_t(0, 50);
        sigma_b_chill2_sp ~ student_t(0, 10);
        mu_b_txchill1_sp ~ student_t(0, 50);
        sigma_b_txchill1_sp ~ student_t(0, 10);

        mu_b_txchill2_sp ~ student_t(0, 50);
        sigma_b_txchill2_sp ~ student_t(0, 10);

	y ~ student_t(yhat, sigma_y);
...
generated quantities{
   ...
   for (n in 1:N)
    ...
    for (n in 1:N)
      y_ppc[n] = student_t_rng(y_ppc[n], sigma_y);
